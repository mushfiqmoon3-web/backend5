...
function getBackendBaseUrl(port) {
    return process.env.BACKEND_BASE_URL || `http://127.0.0.1:${port}`;
}
async function postCron(path, baseUrl) {
    try {
...
export function startCronJobs(port) {
    ...
    console.log(`[cron] jobs scheduled | auto-signal: "${autoSignalSchedule}" | position-monitor: "${positionMonitorSchedule}" | baseUrl: ${baseUrl}`);
}
