...
router.post('/has_role', requireAuth, async (req, res) => {
    ...
    if (targetUserId !== req.user?.id) {
        ...
        const isCurrentUserAdmin = currentUserRoles.some((r) => r.user_id === req.user?.id && r.role === 'admin');
...
router.post('/emergency_stop', requireAuth, async (req, res) => {
    try {
        const environment = (req.body.environment || 'testnet');
        ...
        const openPositions = db.data.positions.filter((p) => p.user_id === userId && p.environment === environment && p.is_open);
        const results = [];
        let closedCount = 0;
        for (const position of openPositions) {
            const product = position.product || 'futures';
            const apiKeyRow = db.data.api_keys.find((k) => k.user_id === userId &&
...
router.post('/approve_deposit', requireAuth, async (req, res) => {
    try {
        const { p_deposit_id, p_admin_id, p_notes } = req.body;
        ...
        const existingBalance = balances.find((b) => b.user_id === deposit.user_id && b.environment === deposit.environment);
...
router.post('/reject_deposit', requireAuth, async (req, res) => {
    try {
        const { p_deposit_id, p_admin_id, p_notes } = req.body;
        if (!p_deposit_id || !p_admin_id) {
            return res.status(400).json({ success: false, error: 'Missing deposit_id or admin_id' });
