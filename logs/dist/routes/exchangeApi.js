...
const createBybitSignature = (timestamp, apiKey, recvWindow, queryString, secret) => {
...
const cancelBinanceSpotSellOrders = async (apiKey, apiSecret, isTestnet, symbol) => {
    ...
    if (!openOrdersResponse.ok || !Array.isArray(openOrders))
        return;
...
router.post('/', requireAuth, async (req, res) => {
    try {
        const { action, exchange, product, environment, symbol } = req.body;
        ...
        const apiKeyData = db.data?.api_keys.find((k) => k.user_id === userId &&
            k.exchange === exchange &&
            k.product === product &&
            ...
            switch (action) {
                ...
                case 'getPositions': {
                    if (product === 'futures') {
                        ...
                        result = (db.data?.positions || []).filter((p) => p.user_id === userId &&
                            p.exchange === exchange &&
                ...
                case 'closePosition': {
                    ...
                    if (product === 'futures') {
                        ...
                        if (!accountResponse.ok) {
                            result = { success: false, error: accountData.msg || 'Failed to fetch spot account' };
                            break;
                        }
                        ...
                        if (closeResponse.ok) {
                            ...
                            const avgExitPrice = (() => {
                                ...
                                if (executedQty > 0 && fromQuote > 0)
                                    return fromQuote / executedQty;
