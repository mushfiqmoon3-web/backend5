...
async function getAccountBalance(exchange, product, environment, apiKey, apiSecret) {
...
async function calculatePositionSize(strategy, currentPrice, apiKey, apiSecret) {
    ...
    const { total: balance } = await getAccountBalance(strategy.exchange, strategy.product, strategy.environment, apiKey, apiSecret);
...
async function executeTrade(strategy, alert, apiKey, apiSecret) {
    const isTestnet = strategy.environment === 'testnet';
    const leverage = alert.leverage || strategy.default_leverage;
    const price = alert.price || 0;
    const positionSide = strategy.strategy_config?.position_side || 'BOTH';
    ...
    if (strategy.exchange === 'binance' && strategy.product === 'futures') {
        // Set leverage first
        await callBinanceApi('/fapi/v1/leverage', apiKey, apiSecret, isTestnet, strategy.product, 'POST', {
            symbol: alert.symbol,
            leverage: leverage.toString(),
        });
        // Place market order
        const side = alert.action === 'buy' ? 'BUY' : 'SELL';
        const orderResult = await callBinanceApi('/fapi/v1/order', apiKey, apiSecret, isTestnet, strategy.product, 'POST', {
            symbol: alert.symbol,
            side,
            type: 'MARKET',
        ...
        // Place market order
        const side = alert.action === 'buy' ? 'Buy' : 'Sell';
        const orderResult = await callBybitApi('/v5/order/create', apiKey, apiSecret, isTestnet, 'POST', {
            category: 'linear',
            symbol: alert.symbol,
            side,
            orderType: 'Market',
...
router.post('/', async (req, res) => {
    try {
        ...
        if (alert.strategy_id) {
            strategy = db.data?.trading_strategies.find((s) => s.id === alert.strategy_id);
        }
        else if (alert.secret) {
            strategy = db.data?.trading_strategies.find((s) => s.webhook_secret === alert.secret);
        }
        ...
        // Get API keys
        const apiKeys = db.data?.api_keys.find((k) => k.user_id === strategy.user_id &&
        ...
        if (alert.action !== 'close') {
            ...
            const dailyTrades = trades.filter((t) => t.user_id === strategy.user_id &&
        ...
        if (alert.action === 'close') {
            ...
            const position = db.data?.positions.find((p) => p.user_id === strategy.user_id &&
        ...
        // Check max positions
        const positionCount = db.data?.positions.filter((p) => p.user_id === strategy.user_id && p.is_open).length;
