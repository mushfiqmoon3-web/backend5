...
async function createMissingProfiles() {
    ...
    for (const user of db.data.app_users) {
        ...
        // Ensure bot_status exists for both environments
        const existingTestnetStatus = db.data.bot_status.find((b) => b.user_id === user.id && b.environment === 'testnet');
        if (!existingTestnetStatus) {
            ...
            console.log(`Created testnet bot_status for user: ${user.email}`);
        }
        const existingMainnetStatus = db.data.bot_status.find((b) => b.user_id === user.id && b.environment === 'mainnet');
        if (!existingMainnetStatus) {
            db.data.bot_status.push({
                id: crypto.randomUUID(),
                user_id: user.id,
                is_running: false,
            ...
            console.log(`Created mainnet bot_status for user: ${user.email}`);
        }
        // Check if gas_fee_balances exist
        const existingTestnetBalance = db.data.gas_fee_balances.find((b) => b.user_id === user.id && b.environment === 'testnet');
        ...
        const existingMainnetBalance = db.data.gas_fee_balances.find((b) => b.user_id === user.id && b.environment === 'mainnet');
